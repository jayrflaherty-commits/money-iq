"""main.py — Money IQ newsletter entry point."""
import argparse, sys
...
def main():
    ...
    subject = raw.get("subject_line", f"Money IQ — {for_date.strftime('%b %d')}")
    preview_text = raw.get("preview_text", "Your daily money move.")
    ...
    post = client.create_post(subject=subject, content_html=html, preview_text=preview_text,
                               draft=args.draft, tags=["money-iq"], send_hour_et=config.SEND_HOUR, send_minute_et=config.SEND_MINUTE)
